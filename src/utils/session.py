...
class Session(SessionData):
    def __init__(self, session_manager, request_handler, subsession):
        if not isinstance(subsession, int) or subsession <= 0:
            raise Exception(_consts.SESSION_INDEX_ERROR)
...
class SessionManager(object):
    def __init__(self, secret, store_options, session_timeout):
        ...
        try:
            self.pool = redis.ConnectionPool(
                host=store_options[_consts.REDIS_HOST_KEY],
                port=store_options[_consts.REDIS_PORT_KEY],
                password=store_options[_consts.REDIS_PASS_KEY])
    ...
    def get(self, subsession, request_handler=None):
        ...
        if request_handler is not None:
            session_id = request_handler.get_secure_cookie(
                _consts.COOKIE_SESSION_KEY)
            hmac_key = request_handler.get_secure_cookie(
                _consts.COOKIE_HMAC_KEY)
    ...
    def set(self, request_handler, session):
        request_handler.set_secure_cookie(_consts.COOKIE_SESSION_KEY,
                                          session.session_id)
        request_handler.set_secure_cookie(_consts.COOKIE_HMAC_KEY,
                                          session.hmac_key)
